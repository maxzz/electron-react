...
const css: React.CSSProperties = {
    ...
    transparent 65%, 65%, #2a7137 70%, 70%, 
    transparent 70%, 70%, #42bc58 74%, 74%, 

    transparent 82%, 82%, #2a7137 87%, 87%,
...
function CardFilename({ path, fullPath, failed, ...rest }: { path: string; fullPath: string; failed?: boolean; } & HTMLAttributes<HTMLElement>) {
    return (
        <div className={`px-2 py-2 ${failed ? 'bg-red-600' : 'bg-neutral-900/20'}`} {...rest}>
            ...
            <div className="w-3 h-3 mt-4 border-gray-900" style={css}></div>
            <div className="w-4 h-4 mt-4 border-gray-900" style={css}></div>
            <div className="w-24 h-24 mt-4 border-gray-900" style={css}></div>
        </div>
    );
}
